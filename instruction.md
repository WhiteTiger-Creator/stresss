...
- Store retained direct source IDs in `dependency_source_ids`. Build lineage depth-first in that order, store it in `dependency_lineage_ids`, keep five unique ids, cap chain depth at six, and calculate causal burst from finalized earlier sources.
...
- `summary.json` uses the literal schema version `service-log-report-v2`; the spec's contract version is documentation metadata and must not be emitted as the schema version.
...
- `service_matrix.json` is the service map itself: every top-level canonical service maps directly to an object containing exactly `debug`, `error`, `info`, and `warn`, including integer zero values. Do not wrap it in `schema_version` or `services`, and do not add schema-order metadata to any output.
- In `repair_audit.json`, `patched_workflow` is the plain string `/app/workflow/export_report.py`. `pre_repair` uses exactly `pipeline_source_sha256` and `pipeline_tokens_present`; `post_repair` uses `flagged_count` and `rerun_flagged_count`. Copy `processing_steps` from the spec verbatim and in order. Token maps use the exact forbidden executable literals `event["timestamp"]` and `level == "error"`.
